...
def compare_strategies(strategy_results: Dict[str, pd.DataFrame], 
...
def calculate_rolling_metrics(results_df: pd.DataFrame, window: int = 252) -> pd.DataFrame:
    """
    ...
    rolling_metrics['rolling_max_drawdown'] = returns.rolling(window).apply(rolling_max_dd)
    
    return rolling_metrics


def generate_performance_report(results_df: pd.DataFrame, trades_summary: Dict,
    ...
    report.append(f"Profit Factor:         {trades_summary.get('profit_factor', 0):.2f}")
