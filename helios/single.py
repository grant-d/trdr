...
DEFAULT_PARAMS = {
    "indicator_trend": "Slope",
    "lookback_long": 50,
    "indicator_volatility": "ATR",
    "lookback_medium": 20,
    "indicator_exhaustion": "RSI",
    "lookback_short": 14,
...
class HeliosTrader:
    ...
    def _get_indicator_functions(self):
        return {
            "Trend": {
                ...
                "MACD": self._calculate_trend_macd,
                "EMA_Crossover": self._calculate_trend_ema_crossover,
            },
            "Volatility": {
                "ATR": self._calculate_volatility_atr,
                "StdDev": self._calculate_volatility_stddev,
                "BollingerBandWidth": self._calculate_volatility_bbw,
            },
            "Exhaustion": {
                "SMADiff": self._calculate_exhaustion_sma_diff,
                "RSI": self._calculate_exhaustion_rsi,
                "Stochastic": self._calculate_exhaustion_stochastic,
            },
        }
    ...
    def _calculate_trend_slope(self, df, lookback):
        ...
        slopes = [
            linregress(np.arange(lookback), df["Close"].iloc[i - lookback : i]).slope
    ...
    def _calculate_trend_macd(self, df, lookback, fast=12, slow=26, signal=9):
        ...
        ema_fast = (
            df["Close"]
            .ewm(span=int(self.best_params["lookback_medium"]), adjust=False)
            .mean()
        )
        ema_slow = (
            df["Close"]
            .ewm(span=int(self.best_params["lookback_long"]), adjust=False)
            .mean()
        )
    ...
    def _load_and_prepare_data(self):
        ...
        try:
            df = pd.read_csv(
                self.config["input_file"], parse_dates=["Date"], index_col="Date"
            )
    ...
    def _create_dollar_bars(self):
        ...
        """
        print(
            f"[Step 2/6] Creating dollar bars with threshold: ${self.config['dollar_threshold']:,}..."
        )
        bars = []
        df = self.raw_df
    ...
    def _calculate_all_factors(self, df, params):
        ...
        try:
            ...
            df_out["Abs_Volatility"] = self.indicator_functions["Volatility"][
                params["indicator_volatility"]
            ](df_out, lookback_map["Volatility"])
            df_out["Trend"] = self.indicator_functions["Trend"][
                params["indicator_trend"]
            ](df_out, lookback_map["Trend"])
            df_out["Exhaustion"] = self.indicator_functions["Exhaustion"][
                params["indicator_exhaustion"]
            ](df_out, lookback_map["Exhaustion"], df_out["Abs_Volatility"])

            # Normalize factors for MSS
            df_out["Norm_Trend"] = (df_out["Trend"].rank(pct=True) - 0.5) * 200
            df_out["Norm_Volatility"] = (
                df_out["Abs_Volatility"].rank(pct=True) - 0.5
            ) * 200
            df_out["Norm_Exhaustion"] = (
                df_out["Exhaustion"].rank(pct=True) - 0.5
            ) * 200
    ...
    def _run_simulation(self, df_with_factors, params):
        ...
        def classify_regime(mss):
            if mss > strong_bull_thresh:
                return "Strong Bull"
            if mss > weak_bull_thresh:
                return "Weak Bull"
            if mss < strong_bear_thresh:
                return "Strong Bear"
            if mss < weak_bear_thresh:
                return "Weak Bear"
            return "Neutral"
        ...
        for index, row in df.iterrows():
            ...
            pos_change_fraction = np.clip(
                target_pos_fraction
                - (total_units * price / current_capital if current_capital > 0 else 0),
                -entry_step,
                entry_step,
            )
            ...
            if abs(total_units) > 1e-9:
                ...
                if total_units > 0:  # Long
                    stop_dist = abs_vol * (
                        stop_mult_strong if regime == "Strong Bull" else stop_mult_weak
                    )
                    tp_dist = abs_vol * (
                        tp_mult_strong if regime == "Strong Bull" else tp_mult_weak
                    )
                    peak_price = max(peak_price, price)
                    stop_loss = (
                        max(stop_loss, peak_price - stop_dist)
                        if stop_loss != 0
                        else peak_price - stop_dist
                    )
                    take_profit = avg_entry_price + tp_dist
                    if (stop_loss > 0 and price <= stop_loss) or (
                        take_profit > 0 and price >= take_profit
                    ):
                        pnl = (price - avg_entry_price) * total_units
                        current_capital += pnl
                        total_units, cost_basis, stop_loss, take_profit, peak_price = (
                            0,
                            0,
                            0,
                            0,
                    ...
                    stop_dist = abs_vol * (
                        stop_mult_strong if regime == "Strong Bear" else stop_mult_weak
                    )
                    tp_dist = abs_vol * (
                        tp_mult_strong if regime == "Strong Bear" else tp_mult_weak
                    )
                    valley_price = min(valley_price, price)
                    stop_loss = (
                        min(stop_loss, valley_price + stop_dist)
                        if stop_loss != 0
                        else valley_price + stop_dist
                    )
                    take_profit = avg_entry_price - tp_dist
                    if (stop_loss > 0 and price >= stop_loss) or (
                        take_profit > 0 and price <= take_profit
            ...
            unrealized_pnl = (
                price - (cost_basis / total_units if total_units != 0 else 0)
            ) * total_units
    ...
    def _fitness_function(self, params, df_train):
        ...
        if sim_results.empty or "Trade_Count" not in sim_results.columns:
            return -1000
    ...
    def _run_ga_optimization(self):
        ...
        for i in range(n_periods):
            ...
            population = [
                {
                    p: self._generate_random_param(p, s)
                    for p, s in PARAMETER_SPACE.items()
                }
            ...
            for gen in range(GA_GENERATIONS):
                ...
                for _ in range(1, GA_POPULATION_SIZE):
                    ...
                    mutated_child[param_to_mutate] = self._generate_random_param(
                        param_to_mutate, PARAMETER_SPACE[param_to_mutate]
                    )
                    new_population.append(mutated_child)
                population = new_population

            print(
                f"  Best Train Fitness (Sortino) for window {i+1}: {best_window_fitness:.4f}"
            )
        ...
        print(
            f"Best parameters found across all walk-forward tests: {self.best_params}"
        )

    def _run_final_backtest(self):
        print(f"\n[Step 4/6] Running final backtest with best parameters...")
        self.results_df = self._calculate_all_factors(
            self.dollar_bars_df, self.best_params
        )
    ...
    def _plot_equity_curve(self):
        ...
        plt.plot(
            ...
            self.results_df["Equity"],
            label="Helios Trader Equity",
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Helios Trader - A quantitative trading algorithm."
    )
    ...
    parser.add_argument(
        "--dollar_threshold",
        type=float,
        default=DEFAULT_DOLLAR_THRESHOLD,
        help="Dollar volume threshold for creating dollar bars.",
    )
    parser.add_argument(
        "--optimize",
        action="store_true",
        help="Run Genetic Algorithm to find optimal parameters.",
    )
    parser.add_argument(
        "--plot", action="store_true", help="Display the equity curve plot."
    )
    parser.add_argument(
        "--allow-shorts",
        action="store_true",
        help="Allow short selling. Default is long-only.",
    )
