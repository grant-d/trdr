...
class Playbook:
    ...
    def _create_default_playbook(self) -> Dict[str, RegimeParameters]:
        ...
        if "BTC" in self.instrument or "ETH" in self.instrument:
            ...
            for regime, params in self.DEFAULT_REGIMES.items():
                ...
                adjusted.weights = {
                    k: v / new_total for k, v in adjusted.weights.items()
                }
                playbook[regime] = adjusted

        elif any(
            stock in self.instrument for stock in ["AAPL", "GOOGL", "MSFT", "SPY"]
        ):
    ...
    def save_playbook(self):
        ...
        data = {regime: params.to_dict() for regime, params in self.regimes.items()}
    ...
    def update_parameters(self, regime: str, parameters: Dict[str, Any]):
        ...
        if regime not in self.regimes:
            ...
            if "stop_loss_atr_multiplier" in parameters:
                current.stop_loss_atr_multiplier = parameters[
                    "stop_loss_atr_multiplier"
                ]
            if "position_size_multiplier" in parameters:
                current.position_size_multiplier = parameters[
    ...
    def get_all_parameters(self) -> Dict[str, Dict]:
        ...
        return {regime: params.to_dict() for regime, params in self.regimes.items()}
...
class PlaybookManager:
    ...
    def update_from_optimization(
        self, context_id: str, optimization_results: Dict[str, Dict[str, Any]]
    ):
