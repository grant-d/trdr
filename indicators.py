...
def calculate_mss(
    df: pd.DataFrame, lookback: int = 20, weights: Optional[Dict[str, float]] = None
    ...
    The weights are fixed (either user-supplied or default). Regimes are classified based on MSS thresholds.
...
def calculate_dynamic_weights(
    regime: str, base_weights: Optional[Dict[str, float]] = None
) -> Dict[str, float]:
    """
    ...
    The output weights are normalized to sum to 1. Use this to get regime-adaptive weights for MSS calculation.
    ...
    if base_weights is None:
        base_weights = {"trend": 0.4, "volatility": 0.3, "exhaustion": 0.3}
...
def calculate_dynamic_mss(
    ...
    for each bar to recalculate MSS and reclassify regimes. This enables regime-adaptive scoring and classification.
