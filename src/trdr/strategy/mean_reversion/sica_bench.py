#!/usr/bin/env python3
"""SICA benchmark script for MeanReversion strategy.

DO NOT MODIFY THE TARGETS IN THIS FILE.
SICA uses this to measure strategy performance. Modify the strategy, not this.

Usage:
  BACKTEST_SYMBOL=crypto:BTC/USD BACKTEST_TIMEFRAME=1d python sica_bench.py
"""
...
def reload_strategy():
    ...
    mod_name = "trdr.strategy.mean_reversion.strategy"
    if mod_name in sys.modules:
        importlib.reload(sys.modules[mod_name])

    from trdr.strategy.mean_reversion.strategy import (
        MeanReversionConfig,
        MeanReversionStrategy,
    )

    return MeanReversionConfig, MeanReversionStrategy
...
def run_backtest(symbol: str, timeframe: str, lookback: int):
    ...
    bt_config = PaperExchangeConfig(
        ...
        default_position_pct=0.5,  # 50% of capital per trade
    )
...
def main():
    ...
    lookback = int(os.environ.get("BACKTEST_LOOKBACK", "3000"))
