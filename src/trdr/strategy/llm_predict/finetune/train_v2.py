...
Usage:
    python -m trdr.strategy.llm_predict.finetune.train_v2
"""
...
TRAIN_DATA_URI = f"gs://{GCS_BUCKET}/finetune/train_v2.jsonl"
VAL_DATA_URI = f"gs://{GCS_BUCKET}/finetune/val_v2.jsonl"

# Training config
TUNED_MODEL_DISPLAY_NAME = "eth-direction-predictor-v2"
...
def upload_data():
    ...
    val_path = data_dir / "val_v2.jsonl"

    if not train_path.exists():
        print(f"ERROR: {train_path} not found. Run generate_data_v2.py first.")
        return False
...
def main():
    ...
    data_dir = Path(__file__).parent / "data"
    if not (data_dir / "train_v2.jsonl").exists():
        print("\nNo training data found. Generate it first:")
        print("  python -m trdr.strategy.llm_predict.finetune.generate_data_v2")
        return
