...
# Model config
SOURCE_MODEL = "gemini-2.5-flash-lite"  # or gemini-2.5-flash-001
TRAIN_DATA_URI = f"gs://{GCS_BUCKET}/finetune/v2/train.jsonl"
VAL_DATA_URI = f"gs://{GCS_BUCKET}/finetune/v2/val.jsonl"
...
def upload_data():
    ...
    if not train_path.exists():
        print(f"ERROR: {train_path} not found. Run generate_data_v1.py first.")
        return False
...
def main():
    ...
    if not (data_dir / "train.jsonl").exists():
        ...
        print("  python -m trdr.strategy.llm_predict.finetune.generate_data_v1")
        return
