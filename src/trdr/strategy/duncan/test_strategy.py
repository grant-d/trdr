"""Backtest tests for Duncan Trailer v2 strategy.

Strategy: Duncan Trailer v2 adaptive trailing stops.
Tests DuncanStrategy from trdr.strategy.

Run with:
  .venv/bin/python -m pytest src/trdr/strategy/duncan/test_strategy.py -v
...
from trdr.strategy.duncan import DuncanConfig, DuncanStrategy
...
SYMBOL, TIMEFRAME, LOOKBACK = get_backtest_env(
    ...
    default_lookback=Duration.parse("30d"),
)
...
def strategy():
    ...
    config = DuncanConfig(
        ...
        atr_period=15,
        multiplier=None,  # Defaults to atr_period
        rsi_weight=0.62,
        min_trail_factor=0.25,
        max_trail_factor=0.95,
        grid_count=3.0,
        close_on_reversal=True,
    )
    return DuncanStrategy(config)
...
def backtest_config():
    ...
    return PaperExchangeConfig(
        ...
        warmup_bars=30,  # atr_period + 10 = 15 + 10 = 25, use 30 for safety
...
def backtest_result(bars, backtest_config, strategy) -> PaperExchangeResult:
    """Run single backtest with Duncan strategy."""
...
class TestAlgoPerformance:
    """Sanity checks for Duncan strategy."""
...
def print_results():
    ...
    async def run():
        ...
        strategy = DuncanStrategy(strategy_config)
