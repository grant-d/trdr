  ...
  async* getDataStream(): AsyncIterableIterator<{ symbol: string; data: any }> {
    while (this.ws?.readyState === WebSocket.OPEN && !this.isDisconnecting) {
      ...
      while (this.messageQueue.length > 0) {
        ...
        // Since symbol is no longer in OhlcvDto, we need to determine it from subscription context
        // For now, use the first subscribed symbol or 'UNKNOWN'
        const symbol = this.subscriptions.keys().next().value || 'UNKNOWN'
        yield { symbol, data }
      }
