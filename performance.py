...
def calculate_returns_metrics(
    ...
    win_rate = (
        float(np.mean(returns_array > 0.0)) * 100 if len(returns_array) > 0 else 0.0
    )
...
def calculate_sharpe_ratio(
    returns: pd.Series, periods_per_year: float, risk_free_rate: float = 0.02
) -> float:
    """
    ...
    excess_returns = returns - (risk_free_rate / periods_per_year)
    return float(excess_returns.mean() / returns.std() * (periods_per_year**0.5))
...
def generate_performance_report(
    ...
    report.append(
        f"Sharpe Ratio:           {green(f'{sharpe:>13.2f}') if sharpe > 1 else yellow(f'{sharpe:>13.2f}') if sharpe > 0.5 else red(f'{sharpe:>13.2f}')}"
    )
    report.append(
        f"Sortino Ratio:          {green(f'{sortino:>13.2f}') if sortino > 2 else yellow(f'{sortino:>13.2f}') if sortino > 1 else red(f'{sortino:>13.2f}')}"
    )
    report.append(
        f"Calmar Ratio:           {green(f'{calmar:>13.2f}') if calmar > 3 else yellow(f'{calmar:>13.2f}') if calmar > 1 else red(f'{calmar:>13.2f}')}"
    )
    ...
    report.append(black("-" * 30))
    report.append(
        f"Total Trades:           {cyan(str(trades_summary.get('total_trades', 0)).rjust(14))}"
    )
    report.append(
        f"Position Cycles:        {str(trades_summary.get('position_cycles', 0)).rjust(14)}"
    )
